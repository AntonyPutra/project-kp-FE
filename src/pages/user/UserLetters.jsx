...
import { FileText, Plus, Clock, CheckCircle, XCircle, Download, Search } from 'lucide-react';
...
const STATUS_CFG = {
  ...
  completed:    { label: 'Selesai',      class: 'badge-cyan',    icon: CheckCircle, color: '#06b6d4' },
};

const STEPS = ['Diajukan', 'Petugas', 'Admin', 'Pimpinan', 'Selesai'];

const INITIAL = [
  { id: 1, tracking: 'SURAT-2026-0101', type: 'domisili',    purpose: 'Keperluan BPJS', status: 'submitted',    step: 1, date: '2026-06-04 09:00' },
  { id: 2, tracking: 'SURAT-2026-0088', type: 'keterangan',  purpose: 'Melamar kerja',  status: 'approved',     step: 3, date: '2026-05-28 14:00' },
];
...
export default function UserLetters() {
  ...
  const submit = () => {
    if (!form.type || !form.purpose) { toast.error('Pilih jenis surat dan keperluan wajib diisi.'); return; }
    const id = Date.now();
    const newLetter = {
      id, tracking: `SURAT-2026-${String(Math.floor(Math.random() * 9000) + 1000)}`,
      type: form.type, purpose: form.purpose,
      status: 'submitted', step: 1,
      date: new Date().toLocaleString('id-ID'),
    };
    setLetters(prev => [newLetter, ...prev]);
    toast.success(`Pengajuan berhasil! Nomor tracking: ${newLetter.tracking}`);
    setShowForm(false);
    ...
    <div className="page-content">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p className="page-subtitle">Ajukan dan pantau status surat masyarakat secara online</p>
        </div>
        ...
        <div className="card" style={{ border: '1px solid rgba(99,102,241,0.3)', background: 'rgba(99,102,241,0.04)', animation: 'fadeIn 0.3s ease' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
            ...
            <button className="btn btn-primary" onClick={submit}>
              <FileText size={14} /> Submit Pengajuan
            </button>
          </div>
        </div>
      )}
      ...
      <div className="card" style={{ padding: '1rem' }}>
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          <div className="input-with-icon" style={{ flex: 1 }}>
            ...
            <input className="input" placeholder="Masukkan nomor tracking (SURAT-2026-XXXX)..." value={tracking} onChange={e => setTracking(e.target.value)} />
          </div>
          <button className="btn btn-secondary">Lacak Surat</button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
        {letters.map((letter, i) => {
          const typeLabel = { domisili: 'Surat Domisili', usaha: 'Surat Keterangan Usaha', keterangan: 'Surat Keterangan', tidak_mampu: 'Surat Tidak Mampu' }[letter.type] || letter.type;
          const st = STATUS_CFG[letter.status];
          return (
            <div key={letter.id} className="card" style={{ animation: `fadeIn 0.3s ${i * 60}ms ease both` }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
                    ...
                    <div>
                      ...
                      <span className="code" style={{ fontSize: '0.6875rem' }}>{letter.tracking}</span>
                    </div>
                  </div>
                  <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', margin: '0 0 0.5rem 0' }}>Keperluan: {letter.purpose}</p>
                  <p style={{ fontSize: '0.75rem', color: 'var(--text-disabled)' }}>Diajukan: {letter.date}</p>
                </div>
              ...
              <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  {STEPS.map((step, idx) => {
                    const done = idx < letter.step;
                    const current = idx === letter.step - 1;
                    return (
                      <React.Fragment key={step}>
                        ...
                        {idx < STEPS.length - 1 && (
                          <div style={{ flex: 1, height: 2, background: idx < letter.step - 1 ? '#6366f1' : 'var(--border)', margin: '0 4px', marginBottom: '14px', transition: 'background 0.3s ease' }} />
                        )}
                      </React.Fragment>
                    );
                  })}
